...
public:
    ...
    yarp::sig::Vector q_current_fake  ;  
...
locoman_control_thread::locoman_control_thread():
    model("bigman",
          "/home/walkman/walkman/drc/iit-bigman-ros-pkg/bigman_urdf/urdf/bigman.urdf",
          "/home/walkman/walkman/drc/iit-bigman-ros-pkg/bigman_srdf/srdf/bigman.srdf"),
    loop_counter(0) ,
    CoM_w_cmd(3, 0.0) ,
    CoM_w_up(3, 0.0) ,
    CoM_w_dw(3, 0.0) ,
...
{   
    ...
    yarp::sig::Vector q_current(model.getJointNames().size(),0.0) ; // = robot.sensePosition();
    model.updateiDyn3Model(q_current, true);
    
   // TODO : adding integral part in the low level control
   
  //-----------------------------
  yarp::sig::Vector q_motor_0(model.getJointNames().size(), 0.0 ) ;                  
  
  yarp::sig::Vector q_des(model.getJointNames().size(), 0.0 ) ;                   
  q_current_fake.resize(model.getJointNames().size(), 0.0);
  ...
  yarp::sig::Vector q_motor_act = q_current;
...
//                            q_motor_init            )  ;
  ...
  size_q = model.getJointNames().size() ; // TODO : verify
  size_u = 6 ;
  size_fc = 24 ;
  kc = 1E6 ;
  Kq.eye() ; Kq = 1000*Kq ;
...
//     int r_hand_c4_index = model.iDyn3_model.getLinkIndex("r_hand_lower_left_link");  // r_foot_lower_right_link
  ...
  l_hand_c1_index = model.iDyn3_model.getLinkIndex("LSoftHand");  // r_foot_upper_left_link // TODO :update the model and change the name of the lik indeces
  l_hand_c2_index = model.iDyn3_model.getLinkIndex("LSoftHand");  // r_foot_upper_right_link
  l_hand_c3_index = model.iDyn3_model.getLinkIndex("LSoftHand");   // r_foot_lower_left_link
  l_hand_c4_index = model.iDyn3_model.getLinkIndex("LSoftHand");  // r_foot_lower_right_link
    
  r_wrist_index   = model.iDyn3_model.getLinkIndex("r_arm_ft") ;
  r_hand_c1_index = model.iDyn3_model.getLinkIndex("RSoftHand");  // r_foot_upper_left_link
  r_hand_c2_index = model.iDyn3_model.getLinkIndex("RSoftHand");  // r_foot_upper_right_link
  r_hand_c3_index = model.iDyn3_model.getLinkIndex("RSoftHand");   // r_foot_lower_left_link
  r_hand_c4_index = model.iDyn3_model.getLinkIndex("RSoftHand");  // r_foot_lower_right_link
...
//--------------------------------------------------------
  ...
  Q_aw_r4_hand(size_q+ 6, size_q + 6)   ; // = Q_ci(J_aw_r_c4_spa_0, T_aw_r_c4_0, fc_r_c4_filt ) ; //(size_q+ 6, size_q + 6) ;
  ...
  Q_aw_s_c_f_rh( 6 , size_q ) ; //  = Q_aw_c.submatrix( 0  , 5,  6,  (Q_aw_c.cols()-1)  ) ;
  //---------------------------------------------------------

  d_fc_des_to_world(size_fc)  ;
  d_EE_r_des(6 ) ;
  d_EE_l_des(6) ;
  ...
  q_ref_ToMove( ( size_q + 6 ) ) ; // TODO verificare dimensioni
  //  
...
////     robot.setReferenceSpeed(max_vel) ;
    ...
    yarp::sig::Vector q_current = q_current_fake;
    
    //std::cout << " sense size : " << q_current.size() << std::endl;
    model.updateiDyn3Model( q_current, true ); //update model first
    
    yarp::sig::Vector tau_current = q_current * 0.0 ;
    yarp::sig::Vector q_motor_side(size_q ) ; //robot.getNumberOfKinematicJoints() ) ;
    // using the q_ in open loop
    model.updateiDyn3Model( q_current_open_loop, true );   // q_current_open_loop    //  q_current
    // by ale 0
    double toc0 = locoman::utils::Toc(tic) ;
    std::cout << "tic-toc0 = " << toc0 << " seconds" << std::endl ;
    // by ale 0
...
//       //std::cout << " q_offset = "  <<  std::endl << q_offset.toString() << std::endl;
  ...
  //--------------------------------------------------------------------//
    ...
    // by ale 1
    double toc1 = locoman::utils::Toc(tic) ;
    std::cout << "tic-toc1 = " << toc1 << " seconds" << std::endl ;
    // by ale 1
...
//   //std::cout << " map_r_hand_fcToSens  =  "  << std::endl << map_r_hand_fcToSens.toString() << std::endl  ;      
    ...
    yarp::sig::Vector fc_r_c_to_robot = map_l_fcToSens_PINV *  ft_r_ankle  ;  
    
    yarp::sig::Vector fc_l_c_to_world =  - 1.0 * fc_l_c_to_robot     ;
    yarp::sig::Vector fc_r_c_to_world =  - 1.0 * fc_r_c_to_robot     ;
    
    if(flag_robot ==1){  // Changing the sign again in case of walkman real robot
...
//     yarp::sig::Vector fc_r_c_hand_to_robot = locoman::utils::Pinv_trunc_SVD( map_r_hand_fcToSens, 1E-10) *  ft_r_wrist  ; 
    yarp::sig::Vector fc_l_c_hand_to_robot = map_l_hand_fcToSens_PINV *  ft_l_wrist  ;  // yarp::math::pinv( map_l_fcToSens, 1E-6)  *  ft_l_ankle     ;
    yarp::sig::Vector fc_r_c_hand_to_robot = map_r_hand_fcToSens_PINV *  ft_r_wrist  ;  // yarp::math::pinv( map_r_fcToSens, 1E-6)  *  ft_r_ankle     ;
...
//-------------------------------------------------------------------------------------------------------------// 
    ...
    // by ale 2
    double toc2 = locoman::utils::Toc(tic) ;
    std::cout << "tic-toc2 = " << toc2 << " seconds" << std::endl ;
    // by ale 2
   //---------- 
   //std::cout << "---------------------------------------------------------------------------" <<  std::endl ; 
//    std::cout << " loop_counter = " <<  loop_counter << std::endl; 
   ...
   yarp::sig::Vector q_motor_init( size_q, 0.0 ) ; // robot.getNumberOfKinematicJoints() ) ;                

   yarp::sig::Vector q_sense_position = q_current_fake ;                        
  ...
  // //std::cout << " q_sense_motor_position = " <<  std::endl << q_sense_motor_position.toString() << std::endl;
   ...
   yarp::sig::Vector q_reconstr_r_arm(right_arm_configuration.length(), 0.0 ) ;
   yarp::sig::Vector q_reconstr_l_arm(left_arm_configuration.length(),  0.0  ) ;
   yarp::sig::Vector q_reconstr_torso(torso_configuration.length(),     0.0  ) ;
   yarp::sig::Vector q_reconstr_r_leg(right_leg_configuration.length(), 0.0 ) ;
   yarp::sig::Vector q_reconstr_l_leg(left_leg_configuration.length(),  0.0  ) ;
...
//                            q_des            );     // building q_des
  ...
  //-----------------------------------------------------------------------------------------
  // Contact Forces
  fc_l_c1_filt = FC_FILTERED.subVector(0,2)  ;  // Applied from the robot to the world
  fc_l_c2_filt = FC_FILTERED.subVector(3,5)  ;
  fc_l_c3_filt = FC_FILTERED.subVector(6,8)  ;
  fc_l_c4_filt = FC_FILTERED.subVector(9,11)  ;

  fc_r_c1_filt = FC_FILTERED.subVector(12,14)  ; 
  fc_r_c2_filt = FC_FILTERED.subVector(15,17)  ; 
  fc_r_c3_filt = FC_FILTERED.subVector(18,20)  ; 
  ...
  T_w_aw_0.resize(4,4); T_w_aw_0.eye();
...
//   yarp::sig::Matrix S_c_r   = S_c_r_T.transposed() ; 
  ...
  // by ale 3
  double toc3 = locoman::utils::Toc(tic) ;
  std::cout << "tic-toc3 = " << toc3 << " seconds" << std::endl ;
  // by ale 3
  //----------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------//
  ...
  // by ale 4
  double toc41 = locoman::utils::Toc(tic) ;
  std::cout << "tic-toc41 = " << toc41 << " seconds" << std::endl ;
  // by ale 4
  ...
  Q_aw_r_c4 = locoman::utils::Q_ci(J_aw_r_c4_spa_0, T_aw_r_c4_0, fc_r_c4_filt ) ; //(size_q+ 6, size_q + 6) ;
   // by ale 4
  double toc42 = locoman::utils::Toc(tic) ;
  std::cout << "tic-toc42 = " << toc42 << " seconds" << std::endl ;
  // by ale 4
  ...
  // by ale 4
  double toc43 = locoman::utils::Toc(tic) ;
  std::cout << "tic-toc43 = " << toc43 << " seconds" << std::endl ;
  // by ale 4
...
//  
  ...
  // by ale 4
  double toc4 = locoman::utils::Toc(tic) ;
  std::cout << "tic-toc4 = " << toc4 << " seconds" << std::endl ;
  // by ale 4
  // 
  //-------------------------------------------------------------------------------------------
  // Useful Definitions
...
//   
 // }
  ...
  if (last_command =="start" || last_command =="resume" ||
...
/*	 last_command == "com_up"   || last_command == "com_dw"   ||
  ...
  yarp::sig::Matrix J_sw_rg_up( 15 , ( model.getJointNames().size() + 6 ));
 ...
 // //std::cout << " Task_sw_rg_fw = " <<  std::endl << Task_sw_rg_fw.toString()  << std::endl;   
  ...
  yarp::sig::Matrix J_sw_rg_dw( 15 , ( model.getJointNames().size() + 6 ));
 ...
 // //std::cout << " Task_sw_rg_up = " <<  std::endl << Task_sw_rg_up.toString()  << std::endl;   
  ...
  yarp::sig::Matrix J_sw_lf_up( 15 , ( model.getJointNames().size() + 6 ));
 ...
 // //std::cout << " Task_sw_lf_fw = " <<  std::endl << Task_sw_lf_fw.toString()  << std::endl;   
  ...
  yarp::sig::Matrix J_sw_lf_dw( 15 , ( model.getJointNames().size() + 6 ));
 ...
 // //std::cout << " Task_sw_lf_dw = " <<  std::endl << Task_sw_lf_dw.toString()  << std::endl;   
  ...
  // J_l_c1_mix_0
  // J_r_c1_mix_0
  yarp::sig::Matrix J_com_up( 15 , ( model.getJointNames().size() + 6 ) );
...
//--------------------------------------------------------------------------------------------    
  ...
  // J_l_c1_mix_0
  // J_r_c1_mix_0
  yarp::sig::Matrix J_com_dw( 15 , ( model.getJointNames().size() + 6 ) );
...
{
    locoman_control_thread thr;
    while(true)
    {
        double t = locoman::utils::Tic();
        thr.run();
        usleep(1000*1000);//sleep(1);
        double t_end = locoman::utils::Toc(t);
        std::cout << "Real Toc: " << t_end << std::endl;
        std::cout.flush();
