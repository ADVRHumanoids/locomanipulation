...
locoman_control_thread::locoman_control_thread( std::string module_prefix, 
    ...
    FC_BIAS_LEFT_SENS(6, 0.0)  ,
    FC_BIAS_RIGHT_SENS(6, 0.0) ,
    err_cl("err.m", std::ofstream::out)
{
  ...
  /*  FC_DES[2] = - mg/8 ;
    ...
    //
    part_hand = 3.0/10.0 ;
    part_foot_left = (6.0/10.0)*( 1.0 - part_hand ) ;
    // link the part parameter (single value linking
    ph->linkParam( PARAM_ID_PART, &part );
    // On the left foot
    FC_DES[2]  = - (mg/12.0 - part_hand*(mg/12.0) + part_foot_left*(mg/12.0) )   ;
    FC_DES[5]  = - (mg/12.0 - part_hand*(mg/12.0) + part_foot_left*(mg/12.0) )   ;
    FC_DES[8]  = - (mg/12.0 - part_hand*(mg/12.0) + part_foot_left*(mg/12.0) )   ;
    FC_DES[11] = - (mg/12.0 - part_hand*(mg/12.0) + part_foot_left*(mg/12.0) )   ;
    FC_DES[14] = - (mg/12.0 - part_hand*(mg/12.0) - part_foot_left*(mg/12.0) )   ;
    FC_DES[17] = - (mg/12.0 - part_hand*(mg/12.0) - part_foot_left*(mg/12.0) )   ;
    FC_DES[20] = - (mg/12.0 - part_hand*(mg/12.0) - part_foot_left*(mg/12.0) )   ;
    FC_DES[23] = - (mg/12.0 - part_hand*(mg/12.0) - part_foot_left*(mg/12.0) )   ; 
    FC_DES[26] = - (mg/12.0 + part_hand*(mg/12.0) )   ;
    FC_DES[29] = - (mg/12.0 + part_hand*(mg/12.0) )   ;
    FC_DES[32] = - (mg/12.0 + part_hand*(mg/12.0) )   ;
    FC_DES[35] = - (mg/12.0 + part_hand*(mg/12.0) )   ; 
    //     
    FC_FILTERED = FC_DES ;
    //                       FC_WINDOW(FC_size, WINDOW_size ) ;
    ...
    if(flag_robot)
    {
     FC_BIAS_LEFT_SENS[0] = - 3.40 ; 
     FC_BIAS_LEFT_SENS[1] = - 11.0 ; 
     FC_BIAS_LEFT_SENS[2] =   4.8 ; 
     FC_BIAS_LEFT_SENS[3] =   0.6 ; 
     FC_BIAS_LEFT_SENS[4] = - 2.56  ; 
     FC_BIAS_LEFT_SENS[5] = - 0.22 ; 

     FC_BIAS_RIGHT_SENS[0] =  - 6.20 ; 
     FC_BIAS_RIGHT_SENS[1] =    4.10 ; 
     FC_BIAS_RIGHT_SENS[2] = -  12.30 ; 
     FC_BIAS_RIGHT_SENS[3] = -  1.50 ; 
     FC_BIAS_RIGHT_SENS[4] = -  2.0  ; 
     FC_BIAS_RIGHT_SENS[5] = -  0.50 ; 
    }
    else
    {
      FC_BIAS_LEFT_SENS.zero()   ;  
      FC_BIAS_RIGHT_SENS.zero()  ; 
    }
...
{   
    std::cout << " Using Robot ? flag_robot = " << flag_robot << std::endl; 
    std::cout << " Using Simulator ?  flag_simulator = " << flag_simulator << std::endl; 
    ...
    //std::cout << " q_motor_0 =  " <<  q_motor_0.toString() << std::endl; 
   ...
   if(init_des)
   {
    ...
    while ( norm(q_motor_act- q_des)>toll_init && fabs(err_0-err_1)>0.000001 ) //( ( (norm(q_motor_act- q_des)>0.01)  )) // &&  ( abs(err_0-err_1)>0.000001 )) )
    {
    err_0 = err_1 ;
    usleep(100*1000) ;
    q_motor_act = senseMotorPosition() ;
    std::cout << " q_motor_act =  " <<  q_motor_act.toString() << std::endl; 
...
{
   yarp::sig::Vector q_motor( robot.getNumberOfJoints() , 0.0 )  ; 
   if(flag_robot)
   {
    //  yarp::sig::Vector q_link = robot.sensePosition() ;
    q_motor = robot.sensePosition() ;
    //return q_motor ; 
   } 
   else
   {
...
{     
    ...
    yarp::sig::Vector tau_right_arm_current(robot.right_arm.getNumberOfJoints()) ; 
    yarp::sig::Vector tau_left_arm_current(robot.left_arm.getNumberOfJoints()) ;
    yarp::sig::Vector tau_torso_current(robot.torso.getNumberOfJoints()) ;
    yarp::sig::Vector tau_right_leg_current(robot.right_leg.getNumberOfJoints()) ;
    yarp::sig::Vector tau_left_leg_current(robot.left_leg.getNumberOfJoints()) ;    
    
    robot.fromIdynToRobot(  tau_current,
                            tau_right_arm_current,
                            tau_left_arm_current,
                            tau_torso_current,
                            tau_right_leg_current,
                            tau_left_leg_current  ) ; 
     
   /* std::cout << " tau_current = " <<  std::endl << tau_current.toString() << std::endl;
    std::cout << " tau_right_arm_current = " <<  std::endl << tau_right_arm_current.toString() << std::endl;
    std::cout << " tau_left_arm_current = " <<  std::endl << tau_left_arm_current.toString() << std::endl;
    std::cout << " tau_torso_current = " <<  std::endl << tau_torso_current.toString() << std::endl;
    std::cout << " tau_right_leg_current = " <<  std::endl << tau_right_leg_current.toString() << std::endl;
    std::cout << " tau_left_leg_current = " <<  std::endl << tau_left_leg_current.toString() << std::endl;*/
...
// Alternative formulations for the iterators      
 std::cout << i->second->getReferenceFrame() << std::endl;     
  ...
  //--------------------------------------------------------------------//
    ...
    if(!robot.senseftSensor("r_wrist", ft_r_wrist)) std::cout << "ERROR READING SENSOR l_ankle" << std::endl; 
...
//----------------------------------------------------------------------------------------------------------//  
    ...
    int RSoftHand_index = model.iDyn3_model.getLinkIndex("RSoftHand") ;

    int rh_c1_index = model.iDyn3_model.getLinkIndex("r_hand_upper_right_link") ;
    int rh_c2_index = model.iDyn3_model.getLinkIndex("r_hand_lower_right_link") ;
    int rh_c3_index = model.iDyn3_model.getLinkIndex("r_hand_upper_left_link") ;
    int rh_c4_index = model.iDyn3_model.getLinkIndex("r_hand_lower_left_link") ;
...
//-------------------------------------------------------------------------------------------------------------// 
    ...
    Kc.eye() ;
    Kc =  1E5*Kc ; //   ;// 1E6*Kc ;    1E8*Kc ;  

    yarp::sig::Matrix Kc_l = Kc.submatrix(  0,11,  0, 11 ) ;
    yarp::sig::Matrix Kc_r = Kc.submatrix( 12,23, 12, 23 ) ;
...
//-------------------------------------------------------------------------------------------------------------    
  /*  yarp::sig::Matrix T_w_l_ankle_0 = model.iDyn3_model.getPosition(l_ankle_index) ;
    ...
    yarp::sig::Matrix T_w_rh_c1_0    = model.iDyn3_model.getPosition(rh_c1_index)    ;    
    yarp::sig::Matrix T_w_rh_c2_0    = model.iDyn3_model.getPosition(rh_c2_index)    ;  
    yarp::sig::Matrix T_w_rh_c3_0    = model.iDyn3_model.getPosition(rh_c3_index)    ;
    yarp::sig::Matrix T_w_rh_c4_0    = model.iDyn3_model.getPosition(rh_c4_index)    ; 
    ...
    yarp::sig::Matrix T_r_wrist_w_0  = iHomogeneous(T_w_r_ankle_0) ;
    yarp::sig::Matrix T_rh_c1_w_0    = iHomogeneous(T_w_rh_c1_0) ;    
    yarp::sig::Matrix T_rh_c2_w_0    = iHomogeneous(T_w_rh_c2_0) ;  
    yarp::sig::Matrix T_rh_c3_w_0    = iHomogeneous(T_w_rh_c3_0) ;
    yarp::sig::Matrix T_rh_c4_w_0    = iHomogeneous(T_w_rh_c4_0) ;   */
    ...
    yarp::sig::Matrix map_l_fcToSens =   fConToSens( l_ankle_index, 
     ...
     yarp::sig::Matrix map_rh_fcToSens = fConToSens( r_wrist_index, 
					              rh_c1_index, 
						      rh_c2_index,
						      rh_c3_index, 
						      rh_c4_index ) ;						      
...
// yarp::sig::Matrix G_pinv_Kc_l = Kc_l*map_l_fcToSens.transposed()* ( luinv( map_l_fcToSens*Kc_l*map_l_fcToSens.transposed()) );						      
//    yarp::sig::Matrix G_pinv_Kc_r = Kc_r*map_r_fcToSens.transposed()* ( luinv( map_r_fcToSens*Kc_r*map_r_fcToSens.transposed()) );
    
//    yarp::sig::Vector fc_l_c_to_robot = G_pinv_Kc_l*( ft_l_ankle - (1.0/1.0)*FC_BIAS_LEFT_SENS )  ; //yarp::math::pinv( map_l_fcToSens, 1E-6)  *  ft_l_ankle     ;
//    yarp::sig::Vector fc_r_c_to_robot = G_pinv_Kc_r*( ft_r_ankle - (1.0/1.0)*FC_BIAS_RIGHT_SENS )  ;// yarp::math::pinv( map_r_fcToSens, 1E-6)  *  ft_r_ankle     ;
...
//    yarp::sig::Vector fc_r_c_to_world = ( - 1.0 * fc_r_c_to_robot ) ;// ;
    ...
    yarp::sig::Vector fc_rh_c_to_robot =  yarp::math::pinv( map_rh_fcToSens, 1E-6) *  ft_r_wrist  ;
    //
    yarp::sig::Vector fc_l_c_to_world  = - 1.0 * fc_l_c_to_robot   ;
    yarp::sig::Vector fc_r_c_to_world  = - 1.0 * fc_r_c_to_robot   ;
    yarp::sig::Vector fc_rh_c_to_world = - 1.0 * fc_rh_c_to_robot  ;
    ...
    fc_to_world_0.setSubvector(fc_l_c_to_world.length() + fc_r_c_to_world.length(), fc_rh_c_to_world ) ;    

   // std::cout << " fc_rh_c_to_world  =  "  << std::endl << fc_rh_c_to_world.toString() << std::endl  ;     
   // std::cout << " fc_to_world_0  =  "  << std::endl << fc_to_world_0.toString() << std::endl  ;     
    ...
    //
    FC_DES_RIGHT_HAND_sensor = map_l_fcToSens* FC_DES.subVector(24,35)  ;    
    //
    // Fc filtering
    //
    ...
    FC_FILTERED = FC_SUM / WINDOW_size ;
    //
    ...
    yarp::sig:: Vector FC_FILTERED_RIGHT_HAND_sensor = map_l_fcToSens* FC_FILTERED.subVector(24,35) ;    
 ...
 /*  std::cout << " FC_WINDOW[0][counter_window+ 1] = " <<  FC_WINDOW[0][(counter_window+ 1)%WINDOW_size]  << std::endl;  */
   ...
   norm(FC_DES_LEFT_sensor- FC_FILTERED_LEFT_sensor) + 
   norm(FC_DES_RIGHT_sensor- FC_FILTERED_RIGHT_sensor)+
   norm(FC_DES_RIGHT_HAND_sensor - FC_FILTERED_RIGHT_HAND_sensor)  << std::endl  ; 
...
/*   std::cout << " fc_l_c_to_world = " <<  std::endl << fc_l_c_to_world.toString() << std::endl; 
   ...
   std::cout << " fc_des_to_world = " <<  std::endl << fc_des_to_world.toString() << std::endl;   */
    ...
    yarp::sig::Vector fc_l_c1 = fc_l_c_to_world.subVector(0,2)  ;  // from the robot to the world
    ...
    yarp::sig::Matrix R_Hand_RSoftHand(3,3) ;
    R_Hand_RSoftHand(0,0) = 0   ;
    R_Hand_RSoftHand(1,0) = 1   ;
    R_Hand_RSoftHand(2,0) = 0   ;
    R_Hand_RSoftHand(0,1) = 0   ;
    R_Hand_RSoftHand(1,1) = 0   ;
    R_Hand_RSoftHand(2,1) = 1   ;
    R_Hand_RSoftHand(0,2) =-1   ;
    R_Hand_RSoftHand(1,2) = 0   ;
    R_Hand_RSoftHand(2,2) = 0   ;

    yarp::sig::Vector fc_rh_c1 = R_Hand_RSoftHand*fc_rh_c_to_world.subVector(0,2)  ; // from the robot to the world
    yarp::sig::Vector fc_rh_c2 = R_Hand_RSoftHand*fc_rh_c_to_world.subVector(3,5)  ; 
    yarp::sig::Vector fc_rh_c3 = R_Hand_RSoftHand*fc_rh_c_to_world.subVector(6,8)  ; 
    yarp::sig::Vector fc_rh_c4 = R_Hand_RSoftHand*fc_rh_c_to_world.subVector(9,11) ;         
    ...
    yarp::sig::Matrix T_waist_aw_0 = T_waist_w_0 * T_w_aw_0   ;
    yarp::sig::Matrix T_aw_waist_0 = iHomogeneous(T_waist_aw_0) ;
//----------------------------------------------
    ...
    yarp::sig::Matrix T_r_wrist_w_0  = iHomogeneous(T_w_r_wrist_0) ;
    yarp::sig::Matrix T_rh_c1_w_0    = iHomogeneous(T_w_rh_c1_0) ;    
    yarp::sig::Matrix T_rh_c2_w_0    = iHomogeneous(T_w_rh_c2_0) ;  
    yarp::sig::Matrix T_rh_c3_w_0    = iHomogeneous(T_w_rh_c3_0) ;
    yarp::sig::Matrix T_rh_c4_w_0    = iHomogeneous(T_w_rh_c4_0) ;      
    ...
    yarp::sig::Matrix T_waist_rh_c1_0 = T_waist_w_0 * T_w_rh_c1_0 ;
    yarp::sig::Matrix T_waist_rh_c2_0 = T_waist_w_0 * T_w_rh_c2_0 ;
    yarp::sig::Matrix T_waist_rh_c3_0 = T_waist_w_0 * T_w_rh_c3_0 ;
    yarp::sig::Matrix T_waist_rh_c4_0 = T_waist_w_0 * T_w_rh_c4_0 ;
  ...
  /*  yarp::sig::Matrix T_b_l_c1_0 = T_waist_l_c1_0 ;  // the waist is the floating base {Waist} = {B}
    ...
    yarp::sig::Matrix J_rh_c1_mix_0( robot.getNumberOfJoints(), ( robot.getNumberOfJoints() + 6 ) ) ; 
    yarp::sig::Matrix J_rh_c2_mix_0( robot.getNumberOfJoints(), ( robot.getNumberOfJoints() + 6 ) ) ; 
    yarp::sig::Matrix J_rh_c3_mix_0( robot.getNumberOfJoints(), ( robot.getNumberOfJoints() + 6 ) ) ; 
    yarp::sig::Matrix J_rh_c4_mix_0( robot.getNumberOfJoints(), ( robot.getNumberOfJoints() + 6 ) ) ;
    ...
    model.iDyn3_model.getJacobian( rh_c1_index, J_r_c1_mix_0, false  ) ; //false= mixed version jacobian //true= body jacobian
    model.iDyn3_model.getJacobian( rh_c2_index, J_r_c2_mix_0, false  ) ; //false= mixed version jacobian //true= body jacobian
    model.iDyn3_model.getJacobian( rh_c3_index, J_r_c3_mix_0, false  ) ; //false= mixed version jacobian //true= body jacobian
    model.iDyn3_model.getJacobian( rh_c4_index, J_r_c4_mix_0, false  ) ; //false= mixed version jacobian //true= body jacobian    
    ...
    yarp::sig::Matrix J_rh_c1_body_0 = Adjoint( Homogeneous(getRot(T_rh_c1_w_0), zero_3 ))* J_rh_c1_mix_0 ;
    yarp::sig::Matrix J_rh_c2_body_0 = Adjoint( Homogeneous(getRot(T_rh_c2_w_0), zero_3 ))* J_rh_c2_mix_0 ;
    yarp::sig::Matrix J_rh_c3_body_0 = Adjoint( Homogeneous(getRot(T_rh_c3_w_0), zero_3 ))* J_rh_c3_mix_0 ;
    yarp::sig::Matrix J_rh_c4_body_0 = Adjoint( Homogeneous(getRot(T_rh_c4_w_0), zero_3 ))* J_rh_c4_mix_0 ;
    ...
    yarp::sig::Matrix J_waist_rh_c1_spa_0 = Adjoint( T_waist_rh_c1_0 )* J_rh_c1_body_0 ;
    yarp::sig::Matrix J_waist_rh_c2_spa_0 = Adjoint( T_waist_rh_c2_0 )* J_rh_c2_body_0 ;
    yarp::sig::Matrix J_waist_rh_c3_spa_0 = Adjoint( T_waist_rh_c3_0 )* J_rh_c3_body_0 ;
    yarp::sig::Matrix J_waist_rh_c4_spa_0 = Adjoint( T_waist_rh_c4_0 )* J_rh_c4_body_0 ;
    
    J_waist_l_c1_spa_0.setSubmatrix(Eye_6, 0 , 0 )  ;  // Spatial Jacobian (Waist Frame)
    ...
    J_waist_rh_c1_spa_0.setSubmatrix(Eye_6, 0 , 0 )  ;
    J_waist_rh_c2_spa_0.setSubmatrix(Eye_6, 0 , 0 )  ;
    J_waist_rh_c3_spa_0.setSubmatrix(Eye_6, 0 , 0 )  ;
    J_waist_rh_c4_spa_0.setSubmatrix(Eye_6, 0 , 0 )  ;
    
//------------------------------------------------------------------------------------------------------------
// Stance and Jacobian Matrices
    
    yarp::sig::Matrix Complete_Jac( n_contacts*B.cols() , size_q + 6) ;
    ...
    Complete_Jac.setSubmatrix( B.transposed()*J_rh_c1_body_0 , 8*B.cols() ,0 )  ;
    Complete_Jac.setSubmatrix( B.transposed()*J_rh_c2_body_0 , 9*B.cols() ,0 )  ;
    Complete_Jac.setSubmatrix( B.transposed()*J_rh_c3_body_0 , 10*B.cols() ,0 )  ;
    Complete_Jac.setSubmatrix( B.transposed()*J_rh_c4_body_0 , 11*B.cols() ,0 )  ;    
...
//   
    ...
    yarp::sig::Matrix Q_rh_c1(size_q+6, size_q+6 ) ;
    yarp::sig::Matrix Q_rh_c2(size_q+6, size_q+6 ) ;
    yarp::sig::Matrix Q_rh_c3(size_q+6, size_q+6 ) ;
    yarp::sig::Matrix Q_rh_c4(size_q+6, size_q+6 ) ;
 ...
 //  yarp::sig::Matrix R_f_no_deriv = Rf_redu( J_c,  S_c,  Q_s,  U_s, Kc) ;// 
  // yarp::sig::Matrix R_u_no_deriv = Ru_redu( J_c, S_c , Q_s,  U_s, Kc) ;
   d_fc_des_to_world = fc_des_to_world - FC_FILTERED ; // -fc_to_world_0 ;  fc_to_world_0
      std::cout << " fc_des_to_world = " <<  std::endl << fc_des_to_world.toString() << std::endl;
      std::cout << " FC_FILTERED = " <<  std::endl << FC_FILTERED.toString() << std::endl;
...
//----------------------------------------------------------------------------------------------------------------
   ...
   // Derivatives  (1/2)  Base in the Waist Frame
     ...
     // Introducing derivative terms: base in the waist frame
 /*   Q_l_c1 = Q_ci(J_waist_l_c1_spa_0, T_waist_l_c1_0, fc_l_c1 ) ;
    ...
    Q_rh_c1 = Q_ci(J_waist_rh_c1_spa_0, T_waist_rh_c1_0, fc_rh_c1 ) ; //(size_q+ 6, size_q + 6) ;
    Q_rh_c2 = Q_ci(J_waist_rh_c2_spa_0, T_waist_rh_c2_0, fc_rh_c2 ) ; //(size_q+ 6, size_q + 6) ;
    Q_rh_c3 = Q_ci(J_waist_rh_c3_spa_0, T_waist_rh_c3_0, fc_rh_c3 ) ; //(size_q+ 6, size_q + 6) ; 
    Q_rh_c4 = Q_ci(J_waist_rh_c4_spa_0, T_waist_rh_c4_0, fc_rh_c4 ) ; //(size_q+ 6, size_q + 6) ;
    
    yarp::sig::Matrix Q_l_tot = Q_l_c1 + Q_l_c2 + Q_l_c3 + Q_l_c4;
    yarp::sig::Matrix Q_r_tot = Q_r_c1 + Q_r_c2 + Q_r_c3 + Q_r_c4;
    yarp::sig::Matrix Q_rh_tot = Q_rh_c1 + Q_rh_c2 + Q_rh_c3 + Q_rh_c4;
    yarp::sig::Matrix Q_c =  Q_l_tot + Q_r_tot + Q_rh_tot ;

   yarp::sig::Matrix U_s_der = Q_c.submatrix( 0 ,        5 ,        0, 5) ;  //U_s_cont
   yarp::sig::Matrix U_j_der = Q_c.submatrix( 6 , (Q_c.rows()-1)  , 0, 5 )  ;   //U_j ;
...
//   yarp::sig::Vector d_q_der_ext = -1.0*  Pinv_trunc_SVD(Rf_ext_der , 1E-1) * d_fc_des_to_world ; 
 ...
 //-----------------------------------------------------------------------------------------------
  ...
  yarp::sig::Matrix J_aw_l_c1_spa_0 = Adjoint( T_aw_waist_0)* J_waist_l_c1_spa_0 ;  // Spatial (Waist) Jacobian
 ...
 //----------------------------------------------------------------------------     
   // Computing derivatives of the COM part
     ...
     R_COM_w_0 = getRot( T_COM_w_0 ) ;
        ...
        //----------------------------------------------------------------------------------------------
    // J_com spatial Auxiliary World
     Jac_COM_body_0_temp    =  Adjoint(Homogeneous(R_COM_w_0, zero_3))   *  Jac_w_COM_0_temp ;
     yarp::sig::Matrix Jac_aw_COM_spa_0 =  Adjoint( T_aw_COM_0   )  * Jac_COM_body_0_temp ;
//         std::cout << " Jac_aw_COM_spa_0 = " <<  std::endl << Jac_aw_COM_spa_0.toString() << std::endl;  

     Jac_aw_COM_spa_0.setSubmatrix(Eye_6,0,0) ;  // spatial on the waist with VKC   
  //            std::cout << " Jac_aw_COM_spa_0 = " <<  std::endl << Jac_aw_COM_spa_0.toString() << std::endl;  

     // Setting Zero the rotational part
  Jac_aw_COM_spa_0.setSubmatrix(zero_3_6q, 3, 0    )       ;   // This is crucial!!!!
    
  yarp::sig::Matrix Q_aw_mg = Q_ci(Jac_aw_COM_spa_0, T_aw_COM_0, mg_COM_0) ;
 //        std::cout << " Q_aw_mg.rows = " <<  std::endl << Q_aw_mg.rows() << std::endl;  
 //        std::cout << " Q_aw_mg.cols = " <<  std::endl << Q_aw_mg.cols() << std::endl;  

   yarp::sig::Matrix U_aw_s_mg = Q_aw_mg.submatrix( 0 ,        5 ,        0, 5) ;
   yarp::sig::Matrix U_aw_j_mg = Q_aw_mg.submatrix( 6 , (Q_aw_mg.rows()-1)  , 0, 5 )  ; //U_j ;  
   yarp::sig::Matrix Q_aw_s_mg = Q_aw_mg.submatrix( 0  ,       5,          6,  (Q_aw_mg.cols()-1)  ) ;
   yarp::sig::Matrix Q_aw_j_mg = Q_aw_mg.submatrix( 6 , (Q_aw_mg.rows()-1)  ,  6,  (Q_aw_mg.cols()-1)  ) ;     

//      std::cout << " U_aw_s_mg = " <<  std::endl << U_aw_s_mg.toString() << std::endl;   
//   std::cout << " Q_aw_s_mg = " <<  std::endl << Q_aw_s_mg.toString() << std::endl;   
   
 // yarp::sig::Matrix Temp_Kc = 1E-1*Kc ; 
 //   std::cout << " Temp_Kc = " <<  std::endl << Temp_Kc.toString() << std::endl;   
 //   Kc.eye() ;
 //   Kc =  1E5*Kc ; // 
      
   
   //---------------------------------------------------------------------
   yarp::sig::Matrix U_aw_s_tot = U_aw_s_cont + U_aw_s_mg ;
   yarp::sig::Matrix Q_aw_s_tot = Q_aw_s_cont + Q_aw_s_mg ;

   yarp::sig::Matrix Rf_redu_aw = Rf_redu( J_c,  S_c,  Q_aw_s_tot,  U_aw_s_tot ,  Kc) ;
   
  // yarp::sig::Vector d_q_aw_1_1 = -1.0*  Pinv_trunc_SVD(Rf_redu_aw , 1E-1) * d_fc_des_to_world ; 
  // yarp::sig::Vector d_q_aw_1_2 = -1.0*  Pinv_trunc_SVD(Rf_redu_aw , 1E-2) * d_fc_des_to_world ; 
   yarp::sig::Vector d_q_aw_1_3 = -1.0*  Pinv_trunc_SVD(Rf_redu_aw , 1E-3) * d_fc_des_to_world ; 
  // yarp::sig::Vector d_q_aw_1_4 = -1.0*  Pinv_trunc_SVD(Rf_redu_aw , 1E-4) * d_fc_des_to_world ; 

   //
   yarp::sig::Matrix Rf_filtered = filter_SVD(Rf_redu_aw,  1E-3);
   yarp::sig::Vector d_q_aw_2_3 = -1.0* Pinv_Regularized(Rf_filtered, 1E3)* d_fc_des_to_world ;
   yarp::sig::Vector d_q_aw_2_4 = -1.0* Pinv_Regularized(Rf_filtered, 1E4)* d_fc_des_to_world ;
   yarp::sig::Vector d_q_aw_2_5 = -1.0* Pinv_Regularized(Rf_filtered, 1E5)* d_fc_des_to_world ;
   yarp::sig::Vector d_q_aw_2_6 = -1.0* Pinv_Regularized(Rf_filtered, 1E6)* d_fc_des_to_world ;
   yarp::sig::Vector d_q_aw_2_7 = -1.0* Pinv_Regularized(Rf_filtered, 1E7)* d_fc_des_to_world ;
   
   
   //   yarp::sig::Vector d_q_aw_1_5 = -1.0*  Pinv_trunc_SVD(Rf_redu_aw , 1E-5) * d_fc_des_to_world ; 

 //----------------------------------------------------------------------------     

//   std::cout << " d_q_aw_1_1 = " <<  std::endl << d_q_aw_1_1.toString() << std::endl;   
//   std::cout << " d_q_aw_1_2 = " <<  std::endl << d_q_aw_1_2.toString() << std::endl; 
   std::cout << " d_q_aw_1_3 = " <<  std::endl << d_q_aw_1_3.toString() << std::endl; 
 //  std::cout << " d_q_aw_1_4 = " <<  std::endl << d_q_aw_1_4.toString() << std::endl; 
 
   std::cout << " d_q_aw_2_3 = " <<  std::endl << d_q_aw_2_3.toString() << std::endl; 
   std::cout << " d_q_aw_2_4 = " <<  std::endl << d_q_aw_2_4.toString() << std::endl; 
   std::cout << " d_q_aw_2_5 = " <<  std::endl << d_q_aw_2_5.toString() << std::endl; 
   std::cout << " d_q_aw_2_6 = " <<  std::endl << d_q_aw_2_6.toString() << std::endl; 
   std::cout << " d_q_aw_2_7 = " <<  std::endl << d_q_aw_2_7.toString() << std::endl; 
...
/*     Jac_COM_body_0_temp    =  Adjoint(Homogeneous(R_COM_w_0, zero_3))   *  Jac_w_COM_0_temp ;
  ...
  //  yarp::sig::Matrix Q_s_der_mg = Q_s_der + Q_s_mg ;
   ...
   yarp::sig::Vector d_q_mg_1_4 = -1.0*  Pinv_trunc_SVD(Rf_redu_mg , 1E-4) * d_fc_des_to_world ; 
   yarp::sig::Vector d_q_mg_1_5 = -1.0*  Pinv_trunc_SVD(Rf_redu_mg , 1E-5) * d_fc_des_to_world ; 
 ...
 //----------------------------------------------------------------------------     
   ...
   std::cout << " d_q_mg_1_4 = " <<  std::endl << d_q_mg_1_4.toString() << std::endl; 
   std::cout << " d_q_mg_1_5 = " <<  std::endl << d_q_mg_1_5.toString() << std::endl;  */
...
/*   yarp::sig::Vector d_q_mg_4_2 = -1.0*  Pinv_trunc_SVD( U_mg , 1E-2) *  S_c* (   F_z* d_fc_des_to_world) ;
 ...
 /*  yarp::sig::Matrix J_comp_uz( Complete_Jac.rows()+1, Complete_Jac.cols()  )  ;  //
   yarp::sig::Matrix J_comp_uz_filt_3 = filter_SVD(J_comp_uz, 1E-3 ) ;
   yarp::sig::Matrix J_comp_uz_filt_4 = filter_SVD(J_comp_uz, 1E-4 ) ;
   yarp::sig::Matrix J_comp_uz_filt_5 = filter_SVD(J_comp_uz, 1E-5 ) ;
   
   yarp::sig::Matrix J_comp_filt_3 = filter_SVD(Complete_Jac, 1E-3 ) ;
   yarp::sig::Matrix J_comp_filt_4 = filter_SVD(Complete_Jac, 1E-4 ) ;
   yarp::sig::Matrix J_comp_filt_5 = filter_SVD(Complete_Jac, 1E-5 ) ;*/
   
   
   
  // J_comp_uz.setSubmatrix( Complete_Jac , 0  ,    0  ) ;
 //  J_comp_uz.setSubmatrix( M_uz , Complete_Jac.rows() ,    0  ) ;
 //  J_comp_uz.setSubmatrix( -1.0*R_u_no_deriv.submatrix(2,2, 0,R_u_no_deriv.cols()-1  )  , Complete_Jac.rows() ,    6  ) ;

 // d_q_mg_3_1 = nullspaceProjection( J_comp_uz, 1E-5  ) *d_q_mg_3_1 ;
   
 // d_q_mg_3_2 = nullspaceProjection( J_comp_uz, 1E-5  ) *d_q_mg_3_2 ;
 // d_q_mg_3_3 = nullspaceProjection( J_comp_uz, 1E-5  ) *d_q_mg_3_3 ;
  
  
/*  yarp::sig::Vector d_q_mg_3_2_3 = nullspaceProjection( J_comp_uz_filt_3, 1E-5  ) *d_q_mg_3_2 ;
  yarp::sig::Vector d_q_mg_3_2_4 = nullspaceProjection( J_comp_uz_filt_4, 1E-5  ) *d_q_mg_3_2 ;
  yarp::sig::Vector d_q_mg_3_2_5 = nullspaceProjection( J_comp_uz_filt_5, 1E-5  ) *d_q_mg_3_2 ;
     std::cout << " d_q_mg_3_2_3 = " <<  std::endl << (d_q_mg_3_2_3).toString() << std::endl;
   std::cout << " d_q_mg_3_2_4 = " <<  std::endl << (d_q_mg_3_2_4).toString() << std::endl;
   std::cout << " d_q_mg_3_2_5 = " <<  std::endl << (d_q_mg_3_2_5).toString() << std::endl;
      
   d_q_mg_3_2_4 = d_q_mg_3_2_4.subVector(6, d_q_mg_3_2_4.length()-1 ) ;   */

 
   
//   d_q_mg_4_2 = nullspaceProjection( J_comp_uz, 1E-5  ) *d_q_mg_4_2 ;

 /* yarp::sig::Vector d_q_mg_4_2_3 = nullspaceProjection( J_comp_filt_3, 1E-5 )  *d_q_mg_4_2 ;
  yarp::sig::Vector d_q_mg_4_2_4 = nullspaceProjection( J_comp_filt_4, 1E-5 )  *d_q_mg_4_2 ;
  yarp::sig::Vector d_q_mg_4_2_5 = nullspaceProjection( J_comp_filt_5, 1E-5 )  *d_q_mg_4_2 ;
     std::cout << " d_q_mg_4_2_3 = " <<  std::endl << (d_q_mg_4_2_3).toString() << std::endl;
   std::cout << " d_q_mg_4_2_4 = " <<  std::endl << (d_q_mg_4_2_3).toString() << std::endl;
   std::cout << " d_q_mg_4_2_5 = " <<  std::endl << (d_q_mg_4_2_3).toString() << std::endl; */
   
   
 //     d_q_mg_4_2_4 = d_q_mg_4_2_4.subVector(6, d_q_mg_4_2_4.length()-1 ) ; 
 
   
   
   
//   d_q_mg_4_3 = nullspaceProjection( J_comp_uz, 1E-5  ) *d_q_mg_4_3 ;
...
//   d_q_mg_4_3 = nullspaceProjection( Complete_Jac, 1E-5 )  *d_q_mg_4_3 ;
 ...
 //  std::cout << " d_q_mg_3_1 = " <<  std::endl << (d_q_mg_3_1).toString() << std::endl;
   std::cout << " d_u_mg_3_2 = " <<  std::endl << (d_u_mg_3_2).toString() << std::endl;
   std::cout << " d_q_mg_3_2 = " <<  std::endl << (d_q_mg_3_2).toString() << std::endl;
 //  std::cout << " d_q_mg_3_3 = " <<  std::endl << (d_q_mg_3_3).toString() << std::endl;  
  
   std::cout << " d_u_mg_4_2 = " <<  std::endl << (d_u_mg_4_2).toString() << std::endl;
   std::cout << " d_q_mg_4_2 = " <<  std::endl << (d_q_mg_4_2).toString() << std::endl;
 //  std::cout << " d_q_mg_4_3 = " <<  std::endl << (d_q_mg_4_3).toString() << std::endl;
    //----------------------------------------------------------------------------------------------------------------
    // Not minimizing du  
     yarp::sig::Vector d_q_mg_5_1 = -1.0*  Pinv_trunc_SVD( Q_s_mg , 1E-1) *  S_c* ( d_fc_des_to_world) ;
     yarp::sig::Vector d_q_mg_5_2 = -1.0*  Pinv_trunc_SVD( Q_s_mg , 1E-2) *  S_c* ( d_fc_des_to_world) ;
     yarp::sig::Vector d_q_mg_5_3 = -1.0*  Pinv_trunc_SVD( Q_s_mg , 1E-3) *  S_c* ( d_fc_des_to_world) ;
     yarp::sig::Vector d_q_mg_5_4 = -1.0*  Pinv_trunc_SVD( Q_s_mg , 1E-4) *  S_c* ( d_fc_des_to_world) ;
     std::cout << " d_q_mg_5_1 = " <<  std::endl << (d_q_mg_5_1).toString() << std::endl;
     std::cout << " d_q_mg_5_2 = " <<  std::endl << (d_q_mg_5_2).toString() << std::endl;
     std::cout << " d_q_mg_5_3 = " <<  std::endl << (d_q_mg_5_3).toString() << std::endl;
     std::cout << " d_q_mg_5_4 = " <<  std::endl << (d_q_mg_5_4).toString() << std::endl;  */


 /*    yarp::sig::Vector d_u_mg_5_2 = -1.0*R_u_no_deriv * d_q_mg_5_2 ;

   yarp::sig::Vector d_q_5_2_long(robot.getNumberOfJoints()+6,  0.0)  ;

   d_q_5_2_long.setSubvector(0, d_u_mg_5_2 ) ;
   d_q_5_2_long.setSubvector(6, d_q_mg_5_2 ) ;
   std::cout << " d_q_5_2_long = " <<  std::endl << (d_q_5_2_long).toString() << std::endl;

   
  d_q_5_2_long = nullspaceProjection( J_comp_uz, 1E-5  ) *d_q_5_2_long ;

  // d_q_5_2_long = nullspaceProjection( Complete_Jac, 1E-5  ) *d_q_5_2_long ;
   d_q_mg_5_2 = d_q_5_2_long.subVector(6, d_q_5_2_long.length()-1 ) ;  
  std::cout << " d_q_mg_5_2 = " <<  std::endl << (d_q_mg_5_2).toString() << std::endl;  */
...
//   writing data on a file
    ...
    // mwasurign weight  ==   norm(FC_FILTERED_LEFT_sensor.subVector(2,2) ) +norm(FC_FILTERED_RIGHT_sensor.subVector(2,2) ) ; 
    // norm( d_fc_des_to_world )   ;  // d_fc_des_to_world    norm( d_fc_des_to_world ) norm( F_z*d_fc_des_to_world ) 
    ...
    // Move something
    q_ref_ToMove_right_arm[0] += .0;  //.025 ; // MAX =  -.025
    q_ref_ToMove_left_arm[0] += .0  ;  //-.025 ;
    ...
    double err_min = 5 ; //15.0 ;
    double err_max = 40 ;  //30.0 ; 
    double alpha = (1.0/(err_max - err_min)) *(  norm( d_fc_des_to_world ) - err_min)  ;
    ...
    std::cout << " alpha = "  << alpha << std::endl; 
     ...
     yarp::sig::Vector d_q_ref_ToMove =  (1.0/1.0 )*alpha* d_q_aw_2_6 ;//d_q_mg_3_2_4  +  //d_q_mg_3_2 + //d_q_mg_5_2 +   // d_q_mg_3_2  +
                                //  (0.0/1.0 )*alpha*d_q_motor_no_deriv_2 ;
                                
				  //d_q_motor_no_deriv_2   ; //   d_q_motor_no_deriv_2_2   ; /*  */
				 
		/*	default_from_sim : q_ref_ToMove = q_ref_ToMove + (1.0/5.0 )*alpha*d_q_mg_4_2  ;
                                 + (1.0/20.0 )*alpha* d_q_motor_no_deriv_2_2  ; */ /*  */
				 

  //  std::cout << " d_q_ref_ToMove = " <<  std::endl << d_q_ref_ToMove.toString() << std::endl;
				 //----------------------------------------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------------------------------------
   // Extreme Waiter (start)
    
  //   yarp::sig::Matrix Ru_redu_aw  = Ru_redu( J_c,  S_c,  Q_aw_s_tot,  U_aw_s_tot ,  Kc) ;
  //   yarp::sig::Matrix Ru_filtered = filter_SVD(Ru_redu_aw,  1E-3);
  //   yarp::sig::Vector d_u_ToMove  = -1.0*Ru_filtered* d_q_ref_ToMove ;
  //yarp::sig::Vector d_q_star_ToMove(  ( robot.getNumberOfJoints() + 6 ), 0.0  ) ;
   
  // d_q_star_ToMove.setSubvector(0, d_u_ToMove  ) ;
  // d_q_star_ToMove.setSubvector(6, d_q_ref_ToMove  ) ;    
 
 /*   // std::cout << " d_u_ToMove   = "        << std::endl << d_u_ToMove.toString()  << std::endl   ;    
    // std::cout << " d_u_ToMove.length() = " << std::endl << d_u_ToMove.length()  << std::endl   ;    
     //std::cout << " d_q_star_ToMove = " << std::endl << d_q_star_ToMove.toString()  << std::endl   ;    
    // std::cout << " d_q_star_ToMove.length() = " << std::endl << d_q_star_ToMove.length()  << std::endl   ;     */

  // int LSoftHand_index = model.iDyn3_model.getLinkIndex("LSoftHand");

   //  // int l_c1_index = l_foot_upper_left_link_index  ;
   
  //    yarp::sig::Matrix T_w_LSoftHand_0    = model.iDyn3_model.getPosition(LSoftHand_index)    ;   
  //    yarp::sig::Matrix T_LSoftHand_w_0 = iHomogeneous(T_w_LSoftHand_0) ;
  //    yarp::sig::Matrix J_LSoftHand_mix_0(  robot.getNumberOfJoints(), ( robot.getNumberOfJoints() + 6 ) ) ; 
    
  //    model.iDyn3_model.getJacobian( LSoftHand_index, J_LSoftHand_mix_0, false  ) ; //false= mixed version jacobian //true= body jacobian
  //    yarp::sig::Matrix J_LSoftHand_body_0 = Adjoint( Homogeneous( getRot( T_LSoftHand_w_0 ), zero_3 ))* J_LSoftHand_mix_0 ; 

  //    yarp::sig::Matrix J_LSoftHand_body_filter_1 = filter_SVD(J_LSoftHand_body_0 , 1E-1)  ;
  //    yarp::sig::Matrix J_LSoftHand_body_filter_2 = filter_SVD(J_LSoftHand_body_0 , 1E-2)  ;
  //    yarp::sig::Matrix J_LSoftHand_body_filter_3 = filter_SVD(J_LSoftHand_body_0 , 1E-3)  ;
  //    yarp::sig::Matrix J_LSoftHand_body_filter_4 = filter_SVD(J_LSoftHand_body_0 , 1E-4)  ;

    // NullSpace Projection Method   
  //    yarp::sig::Vector d_q_orth_1 = nullspaceProjection( J_LSoftHand_body_filter_1 ) * d_q_star_ToMove ;
  //    yarp::sig::Vector d_q_orth_2 = nullspaceProjection( J_LSoftHand_body_filter_2 ) * d_q_star_ToMove ;
  //    yarp::sig::Vector d_q_orth_3 = nullspaceProjection( J_LSoftHand_body_filter_3 ) * d_q_star_ToMove ;
  //    yarp::sig::Vector d_q_orth_4 = nullspaceProjection( J_LSoftHand_body_filter_4 ) * d_q_star_ToMove ;
    
  //    d_q_orth_1  =  d_q_orth_1.subVector( 6,  d_q_orth_1.length()-1 ) ;
  //    d_q_orth_2  =  d_q_orth_2.subVector( 6,  d_q_orth_2.length()-1 ) ;
  //    d_q_orth_3  =  d_q_orth_3.subVector( 6,  d_q_orth_3.length()-1 ) ;
  //    d_q_orth_4  =  d_q_orth_4.subVector( 6,  d_q_orth_4.length()-1 ) ;
    
 //    // std::cout << " d_q_redu_1 = " <<  std::endl << d_q_redu_1.toString() << std::endl; 

    
  /*   // std::cout << " d_q_orth_1 = " << std::endl << d_q_orth_1.toString()  << std::endl   ;    
     // std::cout << " d_q_orth_2 = " << std::endl << d_q_orth_2.toString()  << std::endl   ;    
     // std::cout << " d_q_orth_3 = " << std::endl << d_q_orth_3.toString()  << std::endl   ;    
    //  std::cout << " d_q_orth_4 = " << std::endl << d_q_orth_4.toString()  << std::endl   ;        */
   
     // Augmented Jacobian Method   
  //     yarp::sig::Vector zero_6(6, 0.0) ;    
     
  //   // std::cout << " Rf_filtered = " << std::endl << Rf_filtered.toString()  << std::endl   ;    
  //  //  std::cout << " J_LSoftHand_body_filter_3 = " << std::endl << J_LSoftHand_body_filter_3.toString() << std::endl   ;  

  //     yarp::sig::Vector d_fc_hand(d_fc_des_to_world.length() + 6 )  ;
  //     d_fc_hand.setSubvector( 0 ,  d_fc_des_to_world  )  ;
  //     d_fc_hand.setSubvector( d_fc_des_to_world.length(), zero_6 ) ;
  //    // std::cout << " d_fc_hand = " << std::endl << d_fc_hand.toString()  << std::endl   ;    

  //     yarp::sig::Matrix R_f_J_hand(  Rf_filtered.rows() + 6 ,  Rf_filtered.cols() + 6   )   ;
  //     bool a = R_f_J_hand.setSubmatrix( Rf_filtered , 0, 6   )  ; //  ( (-1.0*Rf_filtered) , 0, 6   )  ;
  //     bool b = R_f_J_hand.setSubmatrix( J_LSoftHand_body_filter_3, Rf_filtered.rows()   , 0 )  ;
   
  //     yarp::sig::Vector d_q_f_h_star_3 =  Pinv_Regularized( R_f_J_hand ,  1E3)* d_fc_hand ;
  //     yarp::sig::Vector d_q_f_h_star_4 =  Pinv_Regularized( R_f_J_hand ,  1E4)* d_fc_hand ;
  //     yarp::sig::Vector d_q_f_h_star_5 =  Pinv_Regularized( R_f_J_hand ,  1E5)* d_fc_hand ;
  //     yarp::sig::Vector d_q_f_h_star_6 =  Pinv_Regularized( R_f_J_hand ,  1E6)* d_fc_hand ;
  //     yarp::sig::Vector d_q_f_h_star_7 =  Pinv_Regularized( R_f_J_hand ,  1E7)* d_fc_hand ;

 /*    // std::cout << " d_q_f_h_star_3 = " << std::endl << d_q_f_h_star_3.toString()  << std::endl   ;    
    //  std::cout << " d_q_f_h_star_4 = " << std::endl << d_q_f_h_star_4.toString()  << std::endl   ;    
    //  std::cout << " d_q_f_h_star_5 = " << std::endl << d_q_f_h_star_5.toString()  << std::endl   ;    
   //   std::cout << " d_q_f_h_star_6 = " << std::endl << d_q_f_h_star_6.toString()  << std::endl   ;      
   //   std::cout << " d_q_f_h_star_7 = " << std::endl << d_q_f_h_star_7.toString()  << std::endl   ;        */
    
  //     yarp::sig::Vector d_q_f_h_3  =  d_q_f_h_star_3.subVector( 6,  d_q_f_h_star_3.length()-1 ) ;
  //     yarp::sig::Vector d_q_f_h_4  =  d_q_f_h_star_4.subVector( 6,  d_q_f_h_star_4.length()-1 ) ;
  //     yarp::sig::Vector d_q_f_h_5  =  d_q_f_h_star_5.subVector( 6,  d_q_f_h_star_5.length()-1 ) ;
  //     yarp::sig::Vector d_q_f_h_6  =  d_q_f_h_star_6.subVector( 6,  d_q_f_h_star_6.length()-1 ) ;
  //     yarp::sig::Vector d_q_f_h_7  =  d_q_f_h_star_7.subVector( 6,  d_q_f_h_star_7.length()-1 ) ;
    
    
   //// yarp::sig::Vector d_q_ref_ToMove_waiter =  d_q_orth_3 ;
  //    yarp::sig::Vector d_q_ref_ToMove_waiter =  d_q_orth_3  ;   // - 1.0*d_q_f_h_7 ;   //    d_q_f_h_6     d_q_ref_ToMove

    // Extreme Waiter (finish)
    //----------------------------------------------------------------------------------------------------------------
    ...
    //---------------------------------------------------------------------------------------------------------------- 
    // Filtering d_q_motor
    yarp::sig::Vector d_q_ref_ToMove_filter(size_q) ;
    d_q_ref_ToMove_filter =   d_q_ref_ToMove ; //   d_q_ref_ToMove_waiter   ;     //
    double     max;
    double     min ;
    double min_abs;
    double d_q_max ;
    d_q_max = 0.0020 ;  // 0.002;    // to use on the COMAN robot:  d_q_max = 0.02;  with t= 10, max_vel = 2.0 
    max = findMax( d_q_ref_ToMove_filter ) ;
    min = findMin( d_q_ref_ToMove_filter ) ;    
    max = std::abs(max) ;
    min_abs = std::abs(min) ;
    if(min_abs>max) max = min_abs;
    if(max>d_q_max)  { d_q_ref_ToMove_filter =  (d_q_ref_ToMove_filter/max)*d_q_max ; } //(d_q_motor_filter/max)  *d_q_max      
    //std::cout << " d_q_ref_ToMove = " <<  std::endl << d_q_ref_ToMove.toString() << std::endl;
   // std::cout << " Temp_dq = " <<  std::endl << Temp_dq.toString() << std::endl;
   //std::cout << " d_q_ref_ToMove_filter = " <<  std::endl << d_q_ref_ToMove_filter.toString() << std::endl;

    d_q_ref_ToMove =  d_q_ref_ToMove_filter ;                   
    std::cout << " d_q_ref_ToMove = " <<  std::endl << d_q_ref_ToMove.toString() << std::endl;

   //   // d_q_ref_ToMove.zero();
   //    q_ref_ToMove = q_ref_ToMove  + d_q_ref_ToMove  ;
    
    
    
    
    
    q_ref_ToMove = q_ref_ToMove  + d_q_ref_ToMove  ;       // + d_q_ref_ToMove  ;
...
void locoman_control_thread::custom_release()
{
