...
//     robot.setReferenceSpeed(0.3) ;
    ...
    q_motor_0 = robot.sensePositionRefFeedback(); // this uses manually imposed joint stiffness values
...
{     
  ...
  yarp::sig::Matrix Kc(size_fc + size_fc/2, size_fc + size_fc/2) ; // considering feet and right arm
  Kc.eye() ;
...
//-------------------------------------------------------------------------------------------------------------// 
  ...
  //------------------------------------------------------------------------------------------------------------
  //  
  // Stance and Complete Jacobian Matrices considerinf feet and right hand
...
//----------------------------------------------------------------------------------------------------//
  ...
  Q_aw_c = Q_aw_l_tot + Q_aw_r_tot ;// + Q_aw_r_hand_tot ;  
...
//   
 ...
 // ------------------------------------------------------------------
  ...
  robot.move( q_ref_ToMove );   
     ...
     // Feet and Right Hand Section
  else if (last_command =="f_rh_sw_no_rf") // feet and right hand => remove forces from the right foot
     {
  std::cout << "Feet and Right Hand => Removing Contact Forces from the Right Foot "<< std::endl ;
 // ------------------------------------------------------------------
  // TODO : desired contact forces
  ...
  // ------------------------------------------------------------------
   // TODO : pinv * d_f_desired 
  // yarp::sig::Vector d_q_f_rh_sw_no_rf =   ;
...
//  std::cout << " norm(d_q_dsp_7) = " <<  std::endl << norm(d_q_dsp_7)  << std::endl;   
  ...
  std::cout << " alpha = "  << alpha << std::endl; 
    ...
    // TODO : moving the robot                                                            
  //q_ref_ToMove = q_motor_side + (1.0/1.0) * d_q_f_rh_sw_no_rf  ;  
  //robot.move( q_ref_ToMove );   
